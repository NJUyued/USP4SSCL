...
print('text anchor shape: {}'.format(text_anchor.shape))
...
for session in range(start_session, int(args.num_classes / args.nb_cl)):
    ...
    if args.dataset == 'cub':
        ...
        class_index = np.arange(session * args.nb_cl, (session + 1) * args.nb_cl)
        X_train, Y_train, unlabeled_data, unlabeled_gt = utils_pytorch.get_data_file_imagenet100(root="/data/taicai/NNCSL/datasets/imagenet100", base_session=True, index=class_index, train=True, unlabel=False, percentage=args.percentage, return_ulb=True, dataset=args.dataset)
        X_valid, Y_valid = utils_pytorch.get_data_file_imagenet100(root="/data/taicai/NNCSL/datasets/imagenet100", base_session=True, index=class_index, train=False, unlabel=False)
